...
You rate a card 1–5. That rating is a gut check on **where JobScout put it**,
not an absolute verdict on the job. So the learning signal is the gap between
your rating and the rating JobScout's rank predicted.
...
points per standard deviation. Positive means JobScout under-weights `k`.
...
card seen. Unknown is absent — the same rule JobScout applies to `points`,
...
as JobScout improves, and that shrinking is the point.
...
**Unscored cards train separately.** A card JobScout declined to score has no
...
- **The target is defined.** The earlier proposal had to invent one, because
  ...
  minus the one JobScout's rank predicted.
...
- **Legible.** Every number reads as one sentence: "cards high in trajectory
  score 0.28 rating points better than JobScout's rank predicted." That is a
...
the fake JobScout ranks without regard to it. After 60 decisions the correction
...
Stored on every decision, used by nothing yet. They map onto JobScout's two
halves — `would_apply` ≈ Fit, `would_get` ≈ P(hire) — which means the honest
next step is to **validate before letting them drive anything**: at ~200
decisions, check whether JobScout's `score` predicts `would_get` independently
